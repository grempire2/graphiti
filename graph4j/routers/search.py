...
from graphiti_client import get_fact_result_from_edge, GraphitiDep

router = APIRouter()


@router.post('/search/nodes', status_code=status.HTTP_200_OK)
async def search_nodes(request: NodeSearchRequest, graphiti: GraphitiDep) -> NodeSearchResponse:
    """
    ...
    # Use the advanced hybrid search with RRF reranking
    # This combines BM25 (keyword) + Cosine Similarity (semantic) search
    results = await graphiti.search_(
        query=request.query,
        config=NODE_HYBRID_SEARCH_RRF,  # The magic happens here!
...
async def search_facts(request: FactSearchRequest, graphiti: GraphitiDep) -> FactSearchResponse:
    """
    ...
    # Use the standard search method with optional center node
