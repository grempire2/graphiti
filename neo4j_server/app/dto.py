...
class AddEpisodeRequest(BaseModel):
    ...
    episode_body: str = Field(..., description='Content of the episode (text or JSON string)')
    llm_client: Literal['groq', 'gemini', 'ollama'] = Field(
        default='groq', description='LLM client to use'
    )
    embedder_client: Literal['gemini', 'ollama'] = Field(
...
class AdvancedSearchResponse(BaseModel):
    edges: list[FactResult] = Field(default_factory=list, description='List of edges found')
    nodes: list[dict] = Field(default_factory=list, description='List of nodes found')
    episodes: list[dict] = Field(default_factory=list, description='List of episodes found')
    communities: list[dict] = Field(default_factory=list, description='List of communities found')
...
def episode_to_dict(episode):
    ...
    return {
        ...
        'source': episode.source.value if hasattr(episode, 'source') and episode.source else None,
        'source_description': episode.source_description if hasattr(episode, 'source_description') else None,
    }
